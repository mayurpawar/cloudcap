...
| Open a **`kitearc-prod`** finding → the **change-freeze** banner (if set during setup) = detect-only; then the **cost** finding (~$133/mo idle spend). | "My production project is under a change-freeze — CloudCap still detects, but proposes no automated changes. And it found real waste: about $133 a month in idle spend, ready to reclaim. And it proves it — those Cloud Run instances are held always-on but running at under one percent CPU. No real traffic is reaching them; you're just paying to keep them warm." | GW, MEM · **Recommender** |
