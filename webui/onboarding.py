...
def _stepper(active_key: str) -> str:
    ...
    for i, (key, label) in enumerate(STEPS):
        ...
        cells += (f'<div class="flex items-center gap-2">'
                  f'<span class="h-6 w-6 rounded-full flex items-center justify-center text-sm font-bold {dot}">'
                  f'{"✓" if done else i + 1}</span>'
                  f'<span class="text-sm {txt}">{esc(label)}</span></div>')
        if i < len(STEPS) - 1:
...
def _shell(active_key: str, title: str, subtitle: str, body: str) -> str:
    ...
    inner = (
        ...
        '<img src="/logo-small.png" alt="CloudCap" class="w-7 h-7"/>'
        '<span class="font-headline font-bold text-xl text-on-surface">CloudCap</span>'
        '<span class="text-sm text-on-surface-variant ml-1">setup</span>'
        '<a href="/logout" class="ml-auto text-sm text-on-surface-variant hover:text-error">sign out</a></div>'
        + stepper +
        ...
        f'<p class="text-base text-on-surface-variant mt-1 mb-6">{esc(subtitle)}</p>'
        + body +
        '</section></div></div>')
    return _page(inner)
...
def _btn(label, primary=True):
    ...
    return f'<button type="submit" class="px-5 py-2.5 rounded-lg text-base font-bold {cls}">{esc(label)}</button>'
...
def _toggle(name, on, label, value="1"):
    ...
    return (f'<label class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-outline-variant/40 '
            ...
            f'<span class="text-base text-on-surface">{esc(label)}</span></label>')
...
def render_welcome(auth):
    ...
    body = (
        f'<p class="text-base text-on-surface leading-relaxed">Hi {esc(name)} — CloudCap is a '
        ...
        + "".join(f'<div class="flex items-start gap-2 text-base text-on-surface-variant">'
...
def render_discover():
    ...
    for folder, items in groups.items():
        rows += (f'<div class="mt-4 first:mt-0"><div class="text-sm font-bold uppercase tracking-wider '
        ...
        for p in items:
            ...
            rows += (f'<label class="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-outline-variant/40 '
                     ...
                     f'<span class="font-mono text-base text-on-surface">{esc(proj)}</span></label>')
        rows += "</div></div>"
    empty = ('<p class="text-base text-on-surface-variant">No projects discovered yet. Grant the CloudCap '
             'service account <b>Browser</b> at your org/folder, then re-discover.</p>') if not projects else ""
    body = (
        '<div class="flex items-center justify-between mb-1">'
        f'<p class="text-base text-on-surface-variant">Discovered <b class="text-on-surface">{len(projects)}</b> '
        ...
        '<button class="text-sm text-primary hover:underline">↻ Re-discover</button></form></div>'
        '<form method="POST" action="/onboarding/sources">'
        f'{empty}{rows}'
        '<div class="mt-8 flex justify-between">'
        '<a href="/onboarding" class="px-5 py-2.5 rounded-lg text-base font-bold bg-surface border '
...
def render_policy():
    ...
    body = (
        ...
        '<div class="text-sm font-bold uppercase tracking-wider text-on-surface-variant mb-2">Governance domains</div>'
        f'<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">{gov_body}</div>'
        '<div class="text-sm font-bold uppercase tracking-wider text-on-surface-variant mt-6 mb-2">'
        'Actions (how we deliver findings)</div>'
        f'<div class="grid grid-cols-1 sm:grid-cols-2 gap-2">{act_body}</div>'
        '<p class="text-sm text-on-surface-variant mt-3">These are org-wide defaults — you can override per project '
        'anytime on the Policy screen.</p>'
        '<div class="mt-8 flex justify-between">'
        '<a href="/onboarding/discover" class="px-5 py-2.5 rounded-lg text-base font-bold bg-surface border '
        'border-outline-variant/50 text-on-surface hover:bg-surface-container">←  Back</a>'
        + _btn("Continue  →") + '</div></form>')
...
def render_integrations():
    ...
    for proj in selected:
        ...
        if repo and chans:
            badge = ('<span class="inline-flex items-center gap-1 text-sm font-bold text-primary">'
                     '<span class="material-symbols-outlined text-base">check_circle</span>Configured</span>')
        elif repo or chans or frozen:
            badge = '<span class="text-sm font-semibold text-tertiary">Partial</span>'
        else:
            badge = '<span class="text-sm text-on-surface-variant">Not set</span>'
        ...
        cards += (
            ...
            f'<span class="font-mono text-base text-on-surface">{esc(proj)}</span>{badge}</summary>'
    ...
    body = (
        '<p class="text-base text-on-surface-variant mb-3">Set up each project — expand it, add a repo (for fix PRs), '
        ...
        '<a href="/onboarding/policy" class="px-5 py-2.5 rounded-lg text-base font-bold bg-surface border '
        'border-outline-variant/50 text-on-surface hover:bg-surface-container">←  Back</a>'
        '<div class="flex items-center gap-4">'
        '<a href="/onboarding/review" class="text-base text-on-surface-variant hover:text-primary">Skip — finish later</a>'
        '<a href="/onboarding/review" class="px-5 py-2.5 rounded-lg text-base font-bold bg-primary text-on-primary '
...
def render_review():
    ...
    for p in selected:
        ...
        if repo and chan:
            configured += 1
            badge = ('<span class="inline-flex items-center gap-1 text-sm font-bold text-primary whitespace-nowrap">'
                     '<span class="material-symbols-outlined text-base">check_circle</span>Configured</span>')
        elif repo or chan or frozen:
            badge = '<span class="text-sm font-semibold text-tertiary whitespace-nowrap">Partial</span>'
        else:
            badge = '<span class="text-sm text-on-surface-variant whitespace-nowrap">Report-only</span>'
        details = []
        if s.get("repo"):
        ...
        det = (f'<div class="text-sm text-on-surface-variant mt-0.5 break-all">{" · ".join(details)}</div>'
               if details else "")
        proj_rows += (
            '<div class="flex items-start justify-between gap-4 px-4 py-2.5 border-b border-outline-variant/30 last:border-0 '
            'hover:bg-surface-container-low">'
            f'<div class="min-w-0"><div class="font-mono text-base text-on-surface">{esc(p)}</div>{det}</div>'
            f'<div class="shrink-0 pt-0.5">{badge}</div></div>')

    def tile(big, sub):
        return (f'<div class="rounded-lg border border-[#d4d4d8] bg-surface p-3 flex flex-col">'
                f'<div class="text-base font-semibold text-on-surface leading-snug">{big}</div>'
                f'<div class="text-sm text-on-surface-variant mt-auto pt-2">{esc(sub)}</div></div>')
    ...
    body = (
        ...
        + tile(f'<span class="text-2xl font-bold text-primary">{len(selected)}</span> '
               f'<span class="text-sm text-on-surface-variant">· {configured} configured</span>', "projects in scope")
        + tile(esc(", ".join(gov) or "none"), "governance")
        + tile(esc(", ".join(pol) or "none"), "actions")
        + '</div>'
        '<div class="text-sm font-bold uppercase tracking-wider text-on-surface-variant mb-2">Projects</div>'
        f'<section class="bg-surface border border-[#d4d4d8] rounded-lg overflow-hidden">{proj_rows}</section>'
        '<p class="text-sm text-on-surface-variant mt-2">Report-only projects still scan — they just have no repo/channel '
        ...
        '<a href="/onboarding/integrations" class="px-5 py-2.5 rounded-lg text-base font-bold bg-surface border '
        'border-outline-variant/50 text-on-surface hover:bg-surface-container">←  Back</a>'
